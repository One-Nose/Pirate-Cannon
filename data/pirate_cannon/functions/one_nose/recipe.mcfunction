execute as @e[type=minecraft:item,nbt={Item:{id:"minecraft:gunpowder",count:1}}] at @s if entity @e[type=minecraft:item,nbt={Item:{id:"minecraft:iron_block",count:1}},distance=..1] positioned ~ ~-0.3 ~ align xyz positioned ~0.5 ~0.5 ~0.5 if block ~ ~ ~ #minecraft:stairs[half=bottom,shape=straight,waterlogged=false] unless entity @e[tag=one_nose_pirate_cannon_main,distance=..3.6] run tag @s add one_nose_pirate_cannon_make
execute at @e[tag=one_nose_pirate_cannon_make] run kill @e[type=minecraft:item,nbt={Item:{id:"minecraft:iron_block",count:1}},distance=..1,limit=1]
